...
for j=1:height
    for l=1:width
        %x(1,l+width*(j-1)) = j;
        %x(2,l+width*(j-1)) = l;
        [x(1,l+width*(j-1)),x(2,l+width*(j-1)),x(3,l+width*(j-1))] = RGB2LUV(i(j,l,1),i(j,l,2),i(j,l,3));
    end
end
%%
%% 
...
zluv(:,:,1)=(reshape(zfilter(1,:),width,height))';
zluv(:,:,2)=(reshape(zfilter(2,:),width,height))';
zluv(:,:,3)=(reshape(zfilter(3,:),width,height))';
